...
- (void)setupProgram {
    ...
    [self setupTexture:@"180" withTexture:self.myTexture0 andTextNum:0];
    [self setupTexture:@"for_test" withTexture:self.myTexture1 andTextNum:1];
...
- (GLuint)setupTexture:(NSString *)fileName withTexture:(GLuint)texture andTextNum:(NSInteger)textureNum{
    ...
    size_t spriteW = CGImageGetWidth(spriteImage);
    size_t spriteH = CGImageGetHeight(spriteImage);
    
    GLubyte *spriteData = (GLubyte *)calloc(spriteW * spriteH *4, sizeof(GLubyte));
    
    CGContextRef spriteContext = CGBitmapContextCreate(spriteData, spriteW, spriteH, 8, spriteW*4, CGImageGetColorSpace(spriteImage), kCGImageAlphaPremultipliedLast);
    
    CGContextDrawImage(spriteContext, CGRectMake(0, 0, spriteW, spriteH), spriteImage);
