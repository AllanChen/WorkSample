...
Boolean	FFTBufferManager::ComputeFFT(char *outFFTData)
{
    if (HasNewAudioData())
    {
        //renderDatas((int32_t *)mAudioBuffer, 3845*20, 76900, 20, 44100, outFFTData);
        renderDatas((int32_t *)mAudioBuffer, 3845 * 20, 3845, 20, 44100, outFFTData);
        OSAtomicDecrement32Barrier(&mHasAudioData);
        OSAtomicIncrement32Barrier(&mNeedsAudioData);
        mAudioBufferCurrentIndex = 0;
        return true;
